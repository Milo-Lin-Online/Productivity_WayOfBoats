...
function renderLogWeek() {
  ...
  const span = LOG_DAY_END - LOG_DAY_START;
  wrap.innerHTML = [6,5,4,3,2,1,0].map(back => {
    const key = keyPlusDays(today, -back);
    ...
    return `<div class="wk-day ${has ? '' : 'empty'} ${key === today ? 'is-today' : ''}"
        ${has ? `onclick="openSavedLog('${key}')"` : ''} title="${escAttr(keyLabel(key, true))}">
      <div class="wk-head">${keyLabel(key).replace(' ', '<br>')}</div>
      <div class="wk-strip">${blocks}</div>
      <div class="wk-foot">${has ? formatMinutes(totalLogged(entries)) : '—'}</div>
    </div>`;
  }).join('');
}
...
function renderLogToc() {
  ...
  // today always has a row, even when empty, so there's somewhere to start
  const isMine = logViewPerson === myPersonId();
  if (isMine && !keys.includes(today)) keys.unshift(today);
  ...
  wrap.innerHTML = keys.map(k => {
    ...
    return `<div class="log-row ${k === logViewDate ? 'active' : ''}" onclick="openSavedLog('${k}')">
      <span class="log-row-date">${k === today ? '📍 Today · ' : ''}${keyLabel(k, true)}</span>
      <span class="log-row-total">${L.entries.length ? `${L.entries.length} · ${formatMinutes(total)}` : 'nothing yet'}</span>
...
function renderLogDetail() {
  ...
  wrap.innerHTML = `
    ...
    <div class="tl-head">
      <div>
        <div class="tl-date">${keyLabel(logViewDate, true)}</div>
        <div class="tl-total">
          ${L.entries.length} ${L.entries.length === 1 ? 'activity' : 'activities'} · ${formatMinutes(total)}
          ${L.edited ? ` · <span style="color:var(--sunset-deep);font-weight:800;">edited ${new Date(L.editedAt).toLocaleDateString()}</span>` : ''}
        </div>
      </div>
      ${mine ? `<button class="btn-primary" onclick="openEntryModal('log','${logViewDate}',null)" style="font-size:13px;">＋ Log an activity</button>` : ''}
    </div>
